...
function getJobsFromBookings() {
  ...
  return bookings.map(b => ({
    ...
    status: b.status === 'Active' ? 'In Progress' : b.status === 'Pending' ? 'Scheduled' : b.status === 'pending_approval' ? 'Pending Approval' : capitalizeStatus(b.status),
...
function statusDot(s) {
  const dots = {
    ...
    "Pending Approval":`<svg width="10" height="10" viewBox="0 0 10 10"><circle cx="5" cy="5" r="5" fill="#d97706"/></svg>`
  };
  return dots[s] || "";
}
        ...
        <td>
          <div class="act-btns">
            ${j.status === 'Pending Approval' ? `
              <button class="act-btn approve" onclick="approveApplication(${realIndex})" title="Approve"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></button>
              <button class="act-btn reject" onclick="rejectApplication(${realIndex})" title="Reject"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
            ` : `
...
function approveApplication(index) {
  const job = jobs[index];
  if (!job || job.originalStatus !== 'pending_approval') return;
  const applicantId = job.booking.applicantId;
  if (!applicantId) return;
  const users = appData.getAllUsers();
  const applicant = users.find(u => u.id === applicantId);
  if (!applicant) return;
  persistBookingChanges(job.bookingId, { status: 'assigned', providerId: applicantId, providerName: applicant.name });
  renderJobs();
}
...
function rejectApplication(index) {
  const job = jobs[index];
  if (!job || job.originalStatus !== 'pending_approval') return;
  persistBookingChanges(job.bookingId, { status: 'Pending' });
  delete job.booking.applicantId;
  renderJobs();
}
...
function confirmAssignment() {
  ...
  let updates = { providerName, status: 'Active' };
  if (typeof appData !== 'undefined') {
    const provider = appData.getAllUsers().find(u => u.name === providerName && providerRoles.includes(u.role));
    if (provider) {
      updates.providerId = provider.id;
      appData.updateUser(provider.id, { status: 'on job' });
    }
  }

  persistBookingChanges(j.bookingId, updates);

  const staffEntry = staff.find(s => s.name === providerName);
  if (staffEntry) {
    staffEntry.status = 'on-job';
    staffEntry.statusLabel = 'On Job';
    staffEntry.sub = `${j.client} · ${j.service}`;
  }
