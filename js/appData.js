...
const DEMO_USERS = [
  ...
  { email: 'mchen@test.com', password: '123456', role: 'caregiver', name: 'Michael Chen' },
...
const DEMO_USERS_LIST = [
  ...
  { id: 'u7', name: 'Michael Chen', email: 'mchen@test.com', phone: '(555) 987-6543', location: 'San Francisco', bookings: 0, totalSpent: '$0', memberSince: 'Jun 5, 2024', status: 'Available', role: 'caregiver' },
...
class AppData {
  ...
  initializeStorageIfEmpty() {
    if (!localStorage.getItem(this.STORAGE_USERS)) {
      ...
      const existingUsers = JSON.parse(localStorage.getItem(this.STORAGE_USERS) || '[]');
      const existingEmails = new Set(existingUsers.map(u => u.email.toLowerCase()));
      const missingUsers = DEMO_USERS_LIST.filter(u => !existingEmails.has(u.email.toLowerCase()));
      if (missingUsers.length) {
        localStorage.setItem(this.STORAGE_USERS, JSON.stringify([...existingUsers, ...missingUsers]));
      }
    }
  ...
  login(email, password) {
    ...
    if (persistedUser) {
      ...
      sessionUser.name = persistedUser.name || sessionUser.name;
      if (persistedUser.status) sessionUser.status = persistedUser.status;
    }
